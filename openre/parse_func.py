"""
Parse call function for Py2.6 and Py2.7 (Python recipe)
http://code.activestate.com/recipes/576671-parse-call-function-for-py26-and-py27/
"""
import ast
...
class StrNodeVisitor(ast.NodeVisitor):
    ...
    def _sequence(self, node, signature):
        ...
        contents = ', '.join([str(visit(elt)) for elt in node.elts])
...
def CallTuple2Str(self):
    ...
    keywords = ', '.join(['%s=%s' % (k, repr(v)) for k, v in kw_values.items()])
...
if __name__ == '__main__':
    ...
    exp = explain('min(1,[5,6],3,4, test="some string", num=1)', ['args', 'keywords', 'func'])
    print exp[0].keywords
